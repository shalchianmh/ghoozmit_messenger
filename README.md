...
|-----------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|اصل 1 - Single Responsibility| پیاده سازی متود های خالی در کلاس هایی که مسئول انجام آن کار نیستند. این کار باعث می‌شود که این کلاس به چیزی وابسته باشد که مسئول انجام آن نیست و اگر در آینده تغییری در آن متود بدهیم باید بدون دلیل این کلاس را هم تغییر بدهیم. همچنین هر کلاس باید یک وظیفه داشته باشد یعنی وظیفه validate کردن نباید در این کلاس ها وجود داشته باشد و باید در کلاس های جداگانه ای پیاده سازی شود.           | باید اینترفیسی که این کلاس ها آن را پیاده سازی می‌کنند را بشکنیم و چند اینترفیس کوچکتر بسازیم که هر کدام مسئولیتی مشخص داشته باشند. همچنین باید وظیفه validate کردن را در کلاس های جداگانه ای پیاده سازی کرد. برای این کار یک اینترفیس جدا به نام ValidatorService مینویسیم و برای هر کدام از سرویس ها آن را جداگانه پیاده سازی میکنیم.                                           |
|اصل 2 - Open/Closed| با وجود یک کلاس اینترفیس و با افزودن یک متود جدید مجبور هستیم تمام کلاس های سرویس را که این اینتر فیس را پیاده سازی کرده اند، تغییر دهیم و متود را که بعضا اصلا به آن احتیاج هم ندارند به این کلاس ها اضافه کنیم. همچنین در کلاس Main داریم از instanceOf استفاده می‌کنیم که این باعث می‌شود در صورت افزودن پیام جدید کد اصلی دچار تغییر شود.                                                  | باید کاری که در قسمت قبل انجام دادیم را به نحوی تغییر دهیم و کلاس interface کلی ای به نام MessageService ایجاد کنیم که ایجاد آن به صورتی است که نوع پیام را هم در ورودی می‌گیرد. همچنین یک کلاس MessageServiceFactory می‌سازیم که شرط های موجودد در کلاس Main را حذف کنیم و این وابستگی را به این کلاس بدهیم. این کلاس وظیفه دارد که بر اساس نوع پیام کلاس متناسب آن را برگرداند. |
|اصل 3 - Liskov Substitution| مشکلی که وجود دارد این است که اگر هرکدام از کلاس های پیاده سازی شده را به جای اینترفیس اصلی استفاده کنیم ممکن است با رفتاری غیرمنتظره مواجه شویم زیرا این کلاس ها هر کدام بنا به مسئولیتی که دارند تعدادی از متود ها را خالی گذاشته اند. همچنین درباره کلاس های مدل Message هم این مشکل وجود دارد و اگر بخواهیم هر کدام از کلاس هی فرزند را به جای Message استفاده کنیم با ارور مواجه می‌شویم. | باید چند اینترفیس داشته باشیم و اینترفیس کلی را بشکنیم. با این کار اگر هر کلاس پیاده سازی را با انترفیسش جایگزین کنیم دقیقا همان رفتار ها را می‌بینیم. همچنین برای کلاسMessage از چنید ریختی استفاده کنیم و متود های getter را به صورت عمومی تعریف کنیم.                                                                                                                          |
|اصل 4 - Interface Segregation| با وجود اینترفیس MessageService همه کلاس هایی که این اینترفیس را پیاده سازی می‌کنند مجبور هستند تا تمام متود ها را پیاده سازی کنند. از این رو متود های خالی یه وجود می‌آید.                                                                                                                                                                                                                    | تعریف این اینترفیس به نحوی که خودش نوع پیام را بفهمد این مشکل را حل می‌کند.                                                                                                                                                                                                                                                                                                       |
|اصل 5 - Dependency Inversion|در کد اولیه در کلاس Main چند شرط داشتیم که نوع پیام را با استفاده از instanceOf بررسی می‌کرد که این باعث ایجاد وابستکی کد سطح بالا با کد های سطح پایین می‌شود.|برای حل این مشکل همان طور که در حل مشکلات قبلی گفته شد یک کلاس MessageServiceFactory نوشتیم که خودش بر اساس نوع پیام سرویس مناسب را برمی‌گرداند.|
</div>
...
در صورتی که اصول SOLID از ابتدا در طراحی پروژه رعایت شده بود، نیاز به افزودن متدهای خالی به کلاس‌های نامرتبط و استفاده از شرط‌های instanceof برای تعیین سرویس مناسب از بین می‌رفت.
با این فرض، از مجموع ۱۰ تغییر اعمال‌شده در مرحله اول، ۴ تغییر (شماره‌های ۳، ۸، ۹ و ۱۰) حذف می‌شد.
بنابراین، با تنها ۶ تغییر واقعی در سطح کد، امکان افزودن سرویس جدید (پیام تلگرام) فراهم می‌شد.

    با توجه به آنچه که انجام دادید، در دو خط توضیح دهید که رعایت اصول شی‌گرایی، چه مزایایی را برای پروژه شما فراهم می‌کند؟

رعایت اصول SOLID باعث افزایش انعطاف‌پذیری پروژه در مواجهه با تغییرات می‌شود؛ به‌طوری‌که می‌توان قابلیت‌های جدید را بدون تغییر در کدهای موجود اضافه کرد.
همچنین این اصول باعث ماژولار شدن کد، کاهش وابستگی بین اجزا و آسان‌تر شدن تست و نگهداری سیستم می‌شوند.
